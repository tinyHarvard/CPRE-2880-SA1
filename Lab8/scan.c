...
/* IR distance threshold in cm.
 * Readings below this value indicate an object is present.
 * The IR sensor is only accurate from ~9 to ~50 cm per the datasheet.
 * ADJUST this value based on your specific CyBot and test field.           */
#define IR_THRESHOLD    50.0f
...
/**
 * average_ir_reading - Position servo and read IR distance using our ADC
 *
 * Calls cyBOT_Scan() once for servo positioning and PING, then uses
 * our custom adc_read_avg() and adc_to_distance() for the IR distance.
 * This replaces the CyBot Scan library's raw IR values per Lab 8.
 *
 * @param angle      Servo angle in degrees (0-180)
 * @param ping_out   Pointer to store the PING reading (cm), or NULL
 * @return           IR distance in cm from our custom ADC code
 */
static float average_ir_reading(int angle, float *ping_out)
{
    ...
    /* Store the PING reading if the caller wants it */
    if (ping_out != NULL)
    {
        *ping_out = scan_data.sound_dist;
    }

    return adc_to_distance(adc_read_avg());
}
 ...
 *   At each angle, read the IR distance using our custom ADC code
 *   (adc_read_avg + adc_to_distance, averaged over 16 samples).
 *   Use a state machine to detect edges:
 *     SEARCHING: looking for IR distance to drop BELOW threshold (object detected)
 *     TRACKING:  looking for IR distance to rise ABOVE threshold (object ended)
 *   Lower distance values mean an object is closer.
 *
...
int scan_objects(detected_obj_t objects[], int max_count)
{
    ...
    for (angle = SCAN_START; angle <= SCAN_END; angle += SCAN_STEP)
    {
        ...
        /* State machine for edge detection using IR distance (cm).
         * Lower distance = object detected (closer object).
         * When the distance drops below IR_THRESHOLD, we have found
         * the leading edge of an object. When it rises back above, we have
         * found the trailing edge.                                           */
        if (!tracking)
        {
            /* SEARCHING state: look for distance below threshold */
        ...
        {
            /* TRACKING state: look for distance to rise above threshold */
            if (avg_ir >= IR_THRESHOLD)
            {
    ...
    for (i = 0; i < obj_count; i++)
    {
        ...
        objects[i].ping_dist = PING_TO_CM(ping_data.sound_dist);
