...
double move_forward(oi_t *sensor_data, double distance_mm)
{
    ...
    while (distance_sum < distance_mm)
    {
        ...
        if (sensor_data->bumpLeft || sensor_data->bumpRight||((2500<sensor_data->cliffFrontLeftSignal || 2500<sensor_data->cliffFrontRightSignal || 2500<sensor_data->cliffLeftSignal || 2500<sensor_data->cliffRightSignal))||((500>sensor_data->cliffFrontLeftSignal || 500>sensor_data->cliffFrontRightSignal || 500>sensor_data->cliffLeftSignal || 500>sensor_data->cliffRightSignal)))
        {
            break;
        }
    }
